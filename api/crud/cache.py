...
async def put_session(cache: Redis, session: User):
    session_model = session.model_dump_json(
        include={"timestamp": True, "credentials": True, "user": {"id": True}}
    )
    await cache.set(name=f"session:{str(session.id)}", value=session_model, ex=3600)
...
async def get_thread(
    cache: Redis, user_id: str, thread_id: str
    ...
    thread = ConversationThread(
        thread_id=thread_id, messages=[], **json.loads(thread_data)
    )
    return thread
